...
#Multiple linear
lm1 <- lm(`PACE Asset Balance` ~ Coupon + `Term (Years)` + `Current Total Debt MTG + PACE` + 
            `Mortgage Balance`  + `Total Project Budget`, data = pace_data)

summary(lm1)

residuals_values <- residuals(lm1)

# Create a data frame to hold the residuals and the predicted values
data_to_plot <- data.frame(Residuals = residuals_values, Predicted = predicted_values)

# Plotting residuals with ggplot2
ggplot(data_to_plot, aes(x = Predicted, y = Residuals)) +
  geom_point(color = 'blue') +
  geom_hline(yintercept = 0, linetype = "dashed", color = 'red') +
  labs(x = "Predicted Values", y = "Residuals", title = "Residuals vs. Predicted Values Plot") +
  theme_minimal()
